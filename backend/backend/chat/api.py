...
def conversation_detail(request, pk):
    ...
    messages_seriazlier = ConversationMessageSerializer(conversation.messages.all(), many=True)
    
    return JsonResponse({'conversation': conversation_serializer.data, 'messages':messages_seriazlier.data}, safe=False, status=status.HTTP_200_OK)
...
def conversations_start(request, user_id):
    ...
    if conversations.count() > 0:
        ...
        return JsonResponse({'success':True, 'conversation_id':conversation.id}, status=status.HTTP_200_OK)
    else:
