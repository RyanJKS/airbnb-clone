            ...
            >
                {categories.map(category => {
                    ...
                    return (
                        <div
                            key={category.id}
                            className={`pb-4 flex flex-col items-center space-y-2 border-b-2 cursor-pointer ${activeCategory === category.id ? 'border-red-500 opacity-100' : 'border-white opacity-60'} hover:opacity-100 hover:border-gray-200`}
                            onClick={() => handleCategorySelect(category.id)}
                        >
                            <IconComponent className={`text-2xl ${activeCategory === category.id ? 'text-red-500' : 'text-black'}`} />
